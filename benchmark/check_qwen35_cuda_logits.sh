...
if [[ -z "$model_path" || ! -f "$model_path" ]]; then
    echo "usage: benchmark/check_qwen35_cuda_logits.sh MODEL.gguf [token_id] [steps] [context] [top_k] [tolerance] [device]" >&2
    exit 2
fi
token_id="${2:-${FORTAI_TOKEN_ID:-9419}}"
steps="${3:-${FORTAI_LOGIT_STEPS:-8}}"
context="${4:-${FORTAI_CONTEXT:-128}}"
top_k="${5:-${FORTAI_LOGIT_TOP_K:-32}}"
tolerance="${6:-${FORTAI_LOGIT_TOLERANCE:-1.0e-2}}"
device="${7:-${CUDA_DEVICE:-0}}"
log_file=$(mktemp)
...
PY
then
    echo "tolerance must be a finite nonnegative number: $tolerance" >&2
    exit 2
fi

FORTAI_TRACE_TOP_LOGITS="$top_k" FORTAI_LLAMA_ORACLE_TOP_K="$top_k" \
CUDA_DEVICE="$device" \
...
if [[ -z "$result_file" || ! -f "$result_file" ]]; then
    echo 'CUDA logit comparison did not produce a result' >&2
    cat "$log_file" >&2
    exit 1
fi
...
if result.get("llama_oracle_top_k") != top_k:
    raise SystemExit("result did not record the requested CUDA llama.cpp top-k oracle")
...
if len(oracle_steps) != int(result["steps"]):
    raise SystemExit("llama.cpp did not return probabilities for every CUDA step")

maximum_error = 0.0
for step, oracle_step in enumerate(oracle_steps):
    oracle_top = oracle_step.get("top_logprobs", [])
    if len(oracle_top) != top_k:
        raise SystemExit(f"llama.cpp returned {len(oracle_top)} top logits at step {step}; expected {top_k}")
    ...
    if any(item is None for item in candidate):
        raise SystemExit(f"FortAI CUDA did not emit {top_k} top logits at step {step}")
    candidate_by_token = {token: delta for token, delta in candidate}
    if candidate[0][0] != int(oracle_step["id"]):
        raise SystemExit(f"top token mismatch at step {step}: FortAI={candidate[0][0]} llama.cpp={oracle_step['id']}")
    if candidate_by_token.keys() != oracle_by_token.keys():
        raise SystemExit(f"top-{top_k} CUDA token set mismatch at step {step}")
    for token, candidate_delta in candidate_by_token.items():
        error = abs(candidate_delta - (oracle_by_token[token] - oracle_max))
        maximum_error = max(maximum_error, error)
        if not math.isfinite(error) or error > tolerance:
            raise SystemExit(
                f"CUDA centered logit mismatch at step {step}, token {token}: "
                f"FortAI={candidate_delta:.9g} llama.cpp={oracle_by_token[token] - oracle_max:.9g} "
                f"abs_error={error:.9g} tolerance={tolerance:.9g}"
            )

print(json.dumps({
    "verdict": "PASS",
    "kind": "llama_cpp_cuda_centered_top_k",
