...
if [[ -z "$model_path" || ! -f "$model_path" ]]; then
    echo "usage: benchmark/check_qwen35_cpu_trace.sh MODEL.gguf [token_id] [steps] [context]" >&2
    exit 2
fi
...
FORTAI_TRACE_TOKENS=1 FORTAI_LLAMA_ORACLE_PROBS=1 \
...
if [[ -z "$result_file" || ! -f "$result_file" ]]; then
    echo 'trace comparison did not produce a result' >&2
    exit 1
fi
